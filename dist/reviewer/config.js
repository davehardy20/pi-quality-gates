 ...
 */
export function loadReviewConfig(directory, opts) {
            ...
            try {
                ...
                if (fbError instanceof Error &&
                    ...
                    try {
                        ...
                        log(`[pi-post-turn-reviewer] Failed to read config from ${globalPath}: ${globalError}`);
                        return { ...DEFAULT_CONFIG };
                    }
                }
                else {
                    log(`[pi-post-turn-reviewer] Failed to read config from ${fallbackPath}: ${fbError}`);
                    return { ...DEFAULT_CONFIG };
                }
            }
        }
        else {
            log(`[pi-post-turn-reviewer] Failed to read config from ${configPath}: ${error}`);
            return { ...DEFAULT_CONFIG };
        }
    }
    let userConfig;
    try {
        ...
        log(`[pi-post-turn-reviewer] Invalid JSON in ${usedPath}, using defaults: ${parseError}`);
        return { ...DEFAULT_CONFIG };
    }
    return {
        model: userConfig.model ?? DEFAULT_CONFIG.model,
        minChangedLines: userConfig.minChangedLines ?? DEFAULT_CONFIG.minChangedLines,
        enabled: userConfig.enabled ?? DEFAULT_CONFIG.enabled,
        maxReReviewPasses: userConfig.maxReReviewPasses ?? DEFAULT_CONFIG.maxReReviewPasses,
        autoFixThreshold: userConfig.autoFixThreshold ?? DEFAULT_CONFIG.autoFixThreshold,
        ...
        allowedBashPatterns: userConfig.allowedBashPatterns ?? DEFAULT_CONFIG.allowedBashPatterns,
        respectGitignore: userConfig.respectGitignore ?? DEFAULT_CONFIG.respectGitignore,
        skipFile: userConfig.skipFile !== undefined
            ? userConfig.skipFile
            : DEFAULT_CONFIG.skipFile,
        allowTestDiscovery: userConfig.allowTestDiscovery ?? DEFAULT_CONFIG.allowTestDiscovery,
        testDiscoveryCommands: {
        ...
        maxChangedLines: userConfig.maxChangedLines ?? DEFAULT_CONFIG.maxChangedLines,
