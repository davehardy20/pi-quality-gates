 ...
 */
export function resolveCommandPath(input, cwd) {
 ...
 */
export function uriToNormalizedPath(uri) {
    try {
 ...
 */
export function normalizeAndSortPaths(filePaths) {
 ...
 *   not exist yet).
 */
export function resolvePath(p, cwd, options) {
 ...
 */
export function isPathMatch(targetPath, pattern, cwd, options) {
    const followSymlinks = options?.followSymlinks ?? true;
    ...
    if (normTarget === normPattern)
        return true;
    if (normTarget.startsWith(normPattern + path.sep))
        return true;
    const relativePath = path.relative(cwd, targetPath);
    const normRelative = relativePath.endsWith(path.sep)
        ? relativePath.slice(0, -1)
        : relativePath;
    if (normRelative === normPattern)
        return true;
    if (normRelative.startsWith(normPattern + path.sep))
        return true;
    const regexPattern = normPattern
        ...
        .replace(/\?/g, ".");
    const sep = path.sep === "\\" ? "\\\\" : "/";
    const regex = new RegExp(`^${regexPattern}$|^${regexPattern}${sep}|${sep}${regexPattern}$|${sep}${regexPattern}${sep}`);
