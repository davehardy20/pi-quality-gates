...
describe("post-turn-reviewer: reviewer helpers", () => {
	...
	it("builds sanitized reviewer commands without task or diff text", () => {
		...
		const command = buildSanitizedReviewerCommand(
			"pi",
			["--mode", "json", "-p", "--append-system-prompt", "/tmp/prompt.md"],
			taskPrompt,
		);

		expect(command).toContain("pi --mode json -p --append-system-prompt");
		expect(command).toContain(
			`[taskPrompt omitted chars=${taskPrompt.length} sha256=`,
		);
		expect(command).not.toContain("preserve private request text");
		expect(command).not.toContain("diff --git");
		expect(command).not.toContain("task-diff-secret");
	});
});
