...
function formatAdvisoryMessage(report: ReviewReport): string {
  const lines = [
...
function formatFixUpMessage(report: ReviewReport): string {
  ...
  lines.push(
    "Fix the CRITICAL issues listed above. Focus on the specific files and lines cited.",
    "After fixing, the reviewer will re-check automatically.",
  );
  return lines.join("\n");
}
...
export default function postTurnReviewerExtension(pi: ExtensionAPI) {
    ...
    try {
      ...
      if (childOutput.timedOut) {
        ...
        const stderrPreview = childOutput.stderr
          ...
          : "";
        ctx.ui.notify(
          `🔍 Post-Turn Reviewer: timed out after ${state.config.timeoutMs / 1000}s (${modelInfo}). Try /reviewer-model to use a faster model, or increase timeout in .pi/reviewer.config.json.${stderrPreview}`,
          "warning",
        );
      ...
      if (!report) {
        ...
        ctx.ui.notify(
          `🔍 Post-Turn Reviewer: could not parse review report from child output.${outputPreview}${hasMarker}`,
          "warning",
        );
  ...
  async function processReport(
    ...
    if (report.status === "CANNOT_REVIEW") {
      ctx.ui.notify(
        `🔍 Post-Turn Reviewer: could not review — ${report.summary}`,
        "warning",
      );
      transition(state, "IDLE");
      return;
