...
export function filterBuiltInIgnoredFiles(filePaths: string[]): string[] {
	return filePaths.filter(
		(filePath) => !isBuiltInIgnoredAgentArtifact(filePath),
	);
}
...
export async function runMarkdownlint(
	...
	try {
		...
		const results = (await lint(lintOptions)) as MarkdownlintResult;
...
function formatMarkdownlintFixInfo(
	fixInfo: MarkdownlintFixInfo | null | undefined,
): string {
	if (!fixInfo) return "";
...
export function formatMarkdownlintResults(results: MarkdownlintResult): string {
	const lines: string[] = [];
...
): Record<string, LinterDefinition> {
...
export async function runQueuedLintChecks(
	...
	const filesByLinter = groupFilesByLinter(
		new Set(filterBuiltInIgnoredFiles(filePaths)),
		config,
	);
	const entries = Array.from(filesByLinter.entries());
	const results: Array<Exclude<Awaited<ReturnType<typeof runLinter>>, null>> =
		[];
	...
	for (const [, groupedFiles] of entries) {
		...
		if (linter.type === "cli" && linter.mode === "project-root") {
			runDirectory = findProjectRoot(
				resolve(groupedFiles[0], ".."),
				linter.rootMarker,
			);
	...
	const validationResults: ValidationOutcome[] = results.map((result) => {
...
async function buildCodeExcerptSection(
	report: string,
	directory: string,
): Promise<string> {
...
async function filterExistingFiles(filePaths: string[]): Promise<string[]> {
	...
	for (const filePath of filePaths) {
		if (isBuiltInIgnoredAgentArtifact(filePath)) continue;
		try {
