...
export type {
	LinterReportSidecarMetadata,
	ParsedReportRecoveryArgs,
	ReportRecoveryMode as LinterReportRecoveryMode,
} from "../shared/report-sidecar.js";
// Re-export runtime detection and shared helpers so existing callers keep working.
export {
	defaultReportSidecarDir as defaultLinterReportSidecarDir,
	deriveSessionId,
...
export async function writeLinterReportSidecar(
	options: LinterReportSidecarWriteOptions,
): Promise<LinterReportSidecarWriteResult> {
	return writeReportSidecar<LinterReportSidecarMetadata>({
...
export async function recoverLinterReportSidecar(
	options: LinterReportRecoveryOptions & { allowFullWithoutAck?: boolean },
): Promise<LinterReportRecoveryResult> {
	return recoverReportSidecar<LinterReportSidecarMetadata>({
...
export function buildSummaryFirstLintMessage(args: {
	...
	const maxFindingsPerFile =
		args.maxFindingsPerFile ?? DEFAULT_MAX_FINDINGS_PER_FILE;
	const maxChars = args.maxChars ?? DEFAULT_SUMMARY_MAX_CHARS;
	const parsed = parseLintReport(args.report, args.cwd);
	const selectedFindings = selectTopFindings(
		parsed.findings,
		maxFindings,
		maxFindingsPerFile,
	);
	const omittedFindings = Math.max(
		0,
		parsed.findings.length - selectedFindings.length,
	);
	const affectedFiles = args.affectedFiles.map((filePath) =>
		displayPath(filePath, args.cwd),
	);
	const linterNames =
		parsed.linterNames.length > 0 ? parsed.linterNames : ["unknown"];
	...
	if (affectedFiles.length > 0) {
		lines.push(
			`Affected files: ${affectedFiles.slice(0, 12).join(", ")}${affectedFiles.length > 12 ? `, +${affectedFiles.length - 12} more` : ""}.`,
		);
	}

	if (args.sidecar?.ok) {
		lines.push(
			`Full redacted report sidecar: ${args.sidecar.metadata.id} (${args.sidecar.metadata.redactedChars} chars).`,
			"Recover manually with /post-turn-linter-report preview, /post-turn-linter-report slice --offset=0 --length=4000, or /post-turn-linter-report full --ack-context-cost.",
		);
	} else {
		const error = args.sidecar?.error ?? "sidecar unavailable";
		lines.push(
			`Full report sidecar unavailable (${truncateText(error, 160)}); raw report omitted from parent context.`,
		);
	}
	...
	if (selectedFindings.length === 0) {
		lines.push(
			"- No path-localized findings could be parsed for the concise summary; recover the sidecar report for full redacted details.",
		);
	} else {
	...
	if (
		...
		if (parsed.lowPriorityFindings > 0) {
			omittedParts.push(
				`${parsed.lowPriorityFindings} low-priority MD013/line-length finding(s)`,
			);
		}
		if (parsed.excerptsOmitted) omittedParts.push("code excerpts omitted");
		lines.push("", `Omitted from parent context: ${omittedParts.join("; ")}.`);
	}

	lines.push(
		"Fix the listed findings first. If the summary is insufficient, recover only the needed sidecar preview/slice before requesting the full report.",
	);
...
function parseLintReport(
	...
	const linterNames = new Set<string>();
	const findings: ParsedLintFinding[] = [];
	...
	for (const rawLine of report.split(/\r?\n/)) {
		...
		if (section) {
			const sectionName =
				section[1]?.replace(/\s+\(\d+ files?\)$/, "") ?? "unknown";
		...
		if (inCodeExcerpts || !line.trim()) continue;
	...
	const lowPriorityFindings = findings.filter(
		(finding) => finding.lowPriority,
	).length;
	return {
...
function parseFindingLine(
	...
	const fixMatch = normalizedTail.match(
		/(?:\s+—\s+fix:\s+|\s+-\s+fix:\s+)(.+)$/i,
	);
	...
	const message = truncateText(
		looksLikeRule ? (firstTokenMatch?.[2] ?? "") : withoutFix,
		260,
	);
	const lowPriority = /(?:^|\b)(MD013|line-length)(?:\b|$)/i.test(
		`${ruleId ?? ""} ${message}`,
	);
...
function selectTopFindings(
	...
	const selected: ParsedLintFinding[] = [];
	...
	for (const { finding } of ranked) {
		...
		if (count >= maxFindingsPerFile) continue;
...
function formatFindingForSummary(finding: ParsedLintFinding): string {
	const location = `${finding.displayPath}:${finding.line}${
