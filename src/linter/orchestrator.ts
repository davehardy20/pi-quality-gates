...
export interface LinterOrchestratorDependencies {
...
interface LinterControllerState {
...
export interface LinterStateSnapshot {
	queuedFiles: number;
	cooldownMs: number;
	reportMode: ReportMode;
	runInProgress: boolean;
...
export interface LinterToolExecutionEvent {
	toolCallId: string;
	toolName?: string;
...
export interface LinterOrchestrator {
	...
	onToolExecutionStart(event: LinterToolExecutionEvent): void;
	onToolExecutionEnd(
		event: LinterToolExecutionEvent,
		ctx: ExtensionContext,
	): Promise<void>;
	onTurnEnd(ctx: ExtensionContext): Promise<void>;
	registerCommands(api: ExtensionAPI): void;
	getStateSnapshot(): LinterStateSnapshot;
}
...
function getFileStats(
	filePath: string,
	statFn: LinterOrchestratorDependencies["statSync"],
...
export function createLinterOrchestrator(
	pi: ExtensionAPI,
	...
	const state: LinterControllerState = {
	...
	function safeSetStatus(ctx: ExtensionContext, text: string) {
		if (ctx.hasUI) {
	...
	async function onTurnEnd(ctx: ExtensionContext): Promise<void> {
	...
	function registerCommands(api: ExtensionAPI): void {
		api.registerCommand("post-turn-linter-run", {
			description:
		...
		api.registerCommand("post-turn-linter-fix", {
			description:
		...
		api.registerCommand("post-turn-linter-report", {
			description:
		...
		api.registerCommand("post-turn-linter-status", {
	...
	function getStateSnapshot(): LinterStateSnapshot {
		return {
			queuedFiles: state.modifiedFiles.size,
			cooldownMs: state.cooldownMs,
			reportMode: state.reportMode,
			runInProgress: state.runInProgress,
			latestReportId: state.latestReportId,
			pendingFixReportId: state.pendingFixReportId,
			latestReportSidecar: state.latestReportSidecar,
			isSubAgentRuntime: state.isSubAgentRuntime,
		};
	}
